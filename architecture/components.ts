...
export const components: ArchComponent[] = [
  ...
  },
  {
    ...
    scaling:
      "Horizontal scalability is a property of the fan-out design (Redis pub/sub), not something bolted on later. LivePulse's own public deployment has only ever run exactly one instance (ADR-008) — but a separate, standalone load test ran two real, unmodified LivePulse backend processes against one shared Redis and measured 100% fan-out delivery (80/80, then 300/300 connections, sub-14ms p99) across both — see the Scaling section below. Production Mode would still need a load balancer with sticky sessions or a connection-routing layer in front, since a client's TCP connection is pinned to one instance even though the fan-out already supports N.",
    codeRefs: [
